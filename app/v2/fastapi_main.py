...
mongodb_user = os.getenv('MONGODB_USER', default='myUser')
username = urllib.parse.quote_plus(mongodb_user)
mongodb_password = os.getenv('MONGODB_PASSWORD', default='myUserPassword')
password = urllib.parse.quote_plus(mongodb_password)
mongodb_host = os.getenv('MONGODB_HOST', default='127.0.0.1')
mongodb_uri = 'mongodb://%s:%s@'+mongodb_host
print("connecting to: ", mongodb_uri)
client = MongoClient(mongodb_uri%(username, password))
db = client.fastapi
mycol = db.cvss_bert
...
'scope', 'confidentiality_impact', 'integrity_impact', 'availability_impact']:
    ...
    post = {"description": vulnerability_description, "predictions": predictions_dict}
